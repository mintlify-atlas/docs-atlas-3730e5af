...
```text
/addons/:transportUrl?/:type?/:catalogId?
```

| Segment | Description |
|---------|-------------|
| `transportUrl` | The transport URL of the addon catalog source (URL-encoded). Omit to view installed addons. |
| `type` | Content type filter, e.g. `movie`, `series` |
| `catalogId` | The catalog identifier from the addon's manifest |

When no segments are provided, the page shows your installed addons. When all three are provided, the page shows the addons available in a remote addon catalog.
